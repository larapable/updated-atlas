...
export async function POST(req) {
    try {
        ...
        try {
            ...
            const result = await WTStrat.postWTStrat(response, department_id);
            ...
            console.error("Error posting W-T:", error);
            return NextResponse.json({ message: "An error occurred while posting W-T." }, { status: 500 });
        }
    } catch (error) {
...
export async function DELETE(req) {
    try {
        ...
        try {
            ...
            const result = await WTStrat.deleteWTStrat(id, department_id);

            // Check if the W-T strategy was successfully deleted
            if (result) {
                // Return a success message
                return NextResponse.json({ message: "W-T strategy deleted successfully." }, { status: 200 });
            } else {
                // Return an error message if deletion failed
                return NextResponse.json({ message: "An error occurred while deleting W-T." }, { status: 500 });
            }
        } catch (error) {
